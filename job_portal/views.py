...
def college_status_counts(request, university_in_charge_id):
    ...
    try:
        enquiry_count = StudentEnquiry.objects.filter(university_in_charge=university_in_charge).count()
        ...
        enquiries_by_month = (
            StudentEnquiry.objects.filter(university_in_charge=university_in_charge)
            .annotate(month=TruncMonth('created_at'))
            .values('month')
            .annotate(count=Count('id'))
            .order_by('month')
